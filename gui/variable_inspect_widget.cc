...
VariableInspectWidget::VariableInspectWidget(QWidget *parent)
    : QWidget(parent),
      variable_view_(new VariableView(this)),
      variable_manager_(new VariableModelDataManager(this))
{
    SetupUI();
    SetupConnections();
}
...
void VariableInspectWidget::SetupUI()
{
    ...
    main_layout->addWidget(variable_view_);
    setLayout(main_layout);

    int size = 100000;
    auto data = variable_manager_->CreateData("test", "{size=" + QString::number(size) + "}", "list");
    for(int i = 0; i < size; i++)
    {
        QString num = QString::number(i);
        auto t = variable_manager_->CreateData("[" + num + "]", num, "int");
        data->AddChild(t);
    }
    variable_view_->variable_model()->AddRootData(data);
